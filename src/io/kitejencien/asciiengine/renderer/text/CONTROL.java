...
public class CONTROL extends TextEffects {
    ...
    private final int _normalDuration = 120;
    private final int _decayDuration = 80;
    private final int _postDecay = 50;
        ...
        for (int i = durationMap.length/2 -1; i > -1; i--) {
            for (int j = 0; j < durationMap[0].length; j++) {
                if (durationMap.length/2 - i <= 2){
                    durationMap[i][j] = new CharParams(keyWord.charAt(2 - (durationMap.length/2 - i)), Math.random());
                }
                if((durationMap.length/2 - i -3) %6 == 0 ){
                    durationMap[i][j] = new CharParams(' ', 1);
                    for (int k = 1; k < 6; k++) {
                        if (i-k > 0)
                            durationMap[i-k][j] = new CharParams(content.charAt(i*durationMap.length + j), Math.random());
                    }
                    if (i-1 > 0)
        ...
        for (int i = 0; i < durationMap.length; i++) {
            for (int j = 0; j < durationMap[0].length; j++) {
                ...
                if (j <= decayIndex && j > decayIndex + _postDecay ){
                    canvas[i][j] = canvas[i][j].chanceOfLiving > (_postDecay - (i-decayIndex))/(double)_postDecay
                            ? canvas[i][j] : new CharParams(' ',1);
                }
